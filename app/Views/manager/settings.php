...
<a href="<?= base_url('manager/settings') ?>" class="nav-item active"><i class="fa-solid fa-sliders"></i> Pengaturan & Backup</a>
<?= $this->endSection() ?>
...
<div class="row g-4">
    ...
    <div class="col-lg-6">
        <div class="card-custom">
            <h5 class="fw-bold mb-4" style="color:#112a1f;"><i class="fa-solid fa-address-card text-muted me-2"></i> Identitas & Kredensial Administrator</h5>

            <form action="<?= base_url('manager/update_profile') ?>" method="POST">
                <?= csrf_field() ?>
                <div class="mb-3">
                    <label class="form-label text-muted" style="font-size:0.75rem; font-weight:700;">NAMA ADMINISTRATOR</label>
                    <input type="text" name="full_name" class="form-control" style="background:#f4f6f8; border:none; padding:12px; border-radius:10px;" value="<?= esc($user['full_name'] ?? $userName) ?>" required>
                </div>
                <div class="mb-3">
                    <label class="form-label text-muted" style="font-size:0.75rem; font-weight:700;">ALAMAT EMAIL RESMI</label>
                    <input type="email" name="email" class="form-control" style="background:#f4f6f8; border:none; padding:12px; border-radius:10px;" value="<?= esc($user['email'] ?? '') ?>" required>
                    <div class="form-text" style="font-size:0.75rem;">Email ini digunakan untuk login portal.</div>
                </div>
                <div class="mb-3">
                    <label class="form-label text-muted" style="font-size:0.75rem; font-weight:700;">NOMOR TELEPON VALID</label>
                    <input type="text" name="phone" class="form-control" style="background:#f4f6f8; border:none; padding:12px; border-radius:10px;" value="<?= esc($user['phone'] ?? '') ?>" required>
                </div>
    ...
    <div class="col-lg-6">
        <div class="card-custom" style="background:linear-gradient(135deg, #2c1011 0%, #4a2123 100%); color:#fff; display:flex; flex-direction:column;">
            ...
            <div class="mt-auto p-4 rounded-3" style="background:rgba(0,0,0,0.2);">
                ...
                <a href="<?= base_url('manager/backup_db') ?>" onclick="return confirm('Mengekspor database utuh (.sql). Proses ini memakan waktu beberapa detik, Lanjutkan?')" class="btn btn-danger w-100 fw-bold border-0 shadow-sm" style="padding:15px; border-radius:10px; background:#e53935; text-transform:uppercase; letter-spacing:1px;">
