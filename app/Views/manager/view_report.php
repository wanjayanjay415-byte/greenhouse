...
<style>
    ...
    @media print { .no-print { display:none !important; } .detail-header { -webkit-print-color-adjust:exact; print-color-adjust:exact; } }
</style>

<!-- Back Button -->
<a href="<?= base_url('manager/reports') ?>" class="btn btn-outline-dark rounded-pill px-4 mb-4 no-print" style="border-width:2px;"><i class="fa-solid fa-arrow-left me-2"></i> Kembali ke Daftar Laporan</a>
    ...
    <div class="d-flex justify-content-between align-items-start mb-4">
        <div>
            ...
            <div style="font-size:0.9rem; opacity:0.8;">
                <i class="fa-regular fa-calendar me-1"></i> <?= date('d M Y', strtotime($report['period_start'])) ?> &ndash; <?= date('d M Y', strtotime($report['period_end'])) ?>
                &nbsp;&middot;&nbsp; Tipe: <?= ucfirst(esc($report['report_type'])) ?>
                &nbsp;&middot;&nbsp; Dibuat: <?= date('d M Y, H:i', strtotime($report['created_at'])) ?>
            </div>
        </div>
        <div class="d-flex gap-2">
            <span class="badge bg-success px-4 py-2 rounded-pill fs-6"><i class="fa-solid fa-check-circle me-1"></i> Tersimpan</span>
        </div>
    </div>
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex gap-2 no-print">
        <?php if($data && !empty($data['detail_orders'])): ?>
            <a href="<?= base_url('manager/export_report_excel/'.$report['id']) ?>" class="btn btn-success rounded-pill px-4 fw-bold shadow-sm">
...
<div class="data-card p-5 text-center">
    ...
    <p class="text-muted fw-bold mb-3">Laporan ini tidak memiliki data terstruktur.</p>
